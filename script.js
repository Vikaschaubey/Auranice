/* =========================================================
   ...
   Product catalogue, shop grid + filters, specSheet drawer,
...
function cardTemplate(p){
  return `
    <article class="productCard reveal" data-id="${p.id}" data-category="${p.category}">
      <div class="pcMedia ${p.photo ? "pcMediaPhoto" : ""}">
        <div class="pcBadges">
          <span class="pcCat" title="${p.categoryLabel}">${p.categoryLabel}</span>
          ${p.rx ? '<span class="pcRx" title="Prescription required">Rx</span>' : ""}
        </div>
        <img src="${p.img}" alt="${p.name} packaging" loading="lazy">
      </div>
      <div class="pcBody">
        <h3 class="pcName">${p.name}</h3>
        <p class="pcTagline">${p.tagline}</p>
        <div class="pcFoot">
          <span class="pcPack">${p.pack}</span>
          <span class="pcLink">View spec sheet
...
function attachCardEvents(){
  document.querySelectorAll(".productCard").forEach(card => {
    ...
    const media = card.querySelector(".pcMedia");
...
function drawerTemplate(p){
  return `
    <div class="specHero">
      <div class="specHeroImgWrap ${p.photo ? "specHeroImgWrapPhoto" : ""}">
        <img src="${p.img}" alt="${p.name} packaging" class="specHeroImg" id="drawerImg">
      </div>
      <p class="specCat">${p.categoryLabel}</p>
      <h2 class="specName">${p.name}</h2>
      <p class="specTagline">${p.tagline}</p>
      <span class="specRxFlag ${p.rx ? "rx" : "otc"}">${p.rx ? "Rx — prescription product" : "OTC — no prescription required"}</span>
    </div>
    <div class="specSheet">
      ${p.description ? `<p class="specSheetTitle">About this product</p><p class="specDescription">${p.description}</p>` : ""}

      <p class="specSheetTitle">Spec sheet</p>
      ...
      <p class="specSheetTitle">Highlights</p>
      <div class="specHighlights">
        ${p.highlights.map(h => `<span class="specChip">${h}</span>`).join("")}
      </div>

      ${p.indications ? `
      <p class="specSheetTitle">Key indications</p>
      <ul class="specIndications">
        ${p.indications.map(i => `<li>${i}</li>`).join("")}
      </ul>` : ""}

      <p class="specSheetTitle">Send enquiry</p>
      <form class="specForm" id="drawerForm">
        <label>Your name
        ...
        <button type="submit" class="btn btnSolid btnBlock">
      ...
      <p class="specDisclaimer">MOQ, price and dispatch shown are indicative and confirmed once we receive your enquiry. Rx items are supplied only against a valid pharmacy / institutional purchase order.</p>
    </div>`;
}
...
document.querySelectorAll(".aboutInner, .categoriesInner, .catCard, .contactInner").forEach(el => {
